...
check_requirements() {
    ...
    if ! command -v pygmentize &> /dev/null; then
        echo -e "${RED}错误: 未找到 pygmentize (minted需要)${NC}"
        echo -e "${YELLOW}请运行: pip install Pygments${NC}"
        exit 1
    fi
...
run_xelatex() {
    ...
    # 注意：xelatex 有警告时也会返回非零退出码，所以不用 set -e
...
compile_latex() {
    ...
    if [ -f "$OUTPUT_DIR/main.bcf" ]; then
        echo "  → 处理参考文献 (biber)..."
        biber "$OUTPUT_DIR/main" >> "$LOG_FILE" 2>&1 || true
    fi
...
clean_temp() {
    ...
    rm -rf "$OUTPUT_DIR"/*.aux "$OUTPUT_DIR"/*.log "$OUTPUT_DIR"/*.toc \
           "$OUTPUT_DIR"/*.out "$OUTPUT_DIR"/*.bbl "$OUTPUT_DIR"/*.bcf \
           "$OUTPUT_DIR"/*.blg "$OUTPUT_DIR"/*.run.xml "$OUTPUT_DIR"/_minted-* \
           2>/dev/null
...
main() {
    ...
    echo -e "${GREEN}======================================${NC}"
    echo -e "${GREEN}  强化学习笔记 - LaTeX 编译${NC}"
    echo -e "${GREEN}======================================${NC}"
    echo
