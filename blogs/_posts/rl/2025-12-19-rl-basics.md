...
\end{tikzpicture}
-->
![Agent-Environment 交互循环]({{ site.baseurl }}/assets/figures/rl-agent-env-loop.svg)
...
值得注意的是，近年来 RL 最成功的应用可能不是游戏，而是 [LLM 对齐]({{ site.baseurl }}/llm-alignment-part1/)——详见本系列第五、六篇。
...
\end{tikzpicture}
-->
![MDP 示例]({{ site.baseurl }}/assets/figures/rl-mdp-example.svg)
...
\end{tikzpicture}
-->
![轨迹生成过程]({{ site.baseurl }}/assets/figures/rl-trajectory.svg)
...
\end{tikzpicture}
-->
![回报计算]({{ site.baseurl }}/assets/figures/rl-return-calc.svg)
...
3. **轨迹期望**：$J(\pi) = \mathbb{E}\_{\tau \sim \pi} \left[ \sum\_{t=0}^{T} \gamma^t r\_t \right]$
...
<!-- tikz-source: rl-methods-tree
\begin{tikzpicture}[
    node/.style={draw, rounded corners, fill=blue!15, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\small},
    leaf/.style={draw, rounded corners, fill=green!15, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\small},
    arrow/.style={->, thick, >=stealth}
]
    % Root
    \node[node, fill=orange!20] (rl) at (0, 0) {RL Methods};

    % Level 1
    \node[node] (vb) at (-4, -1.5) {Value-Based\\Learn $Q^*$};
    \node[node] (pb) at (0, -1.5) {Policy-Based\\Learn $\pi_\theta$};
    \node[node] (ac) at (4, -1.5) {Actor-Critic\\Learn both};

    % Level 2
    \node[leaf] (dqn) at (-4, -3) {DQN, Q-Learning};
    \node[leaf] (pg) at (0, -3) {REINFORCE, TRPO};
    \node[leaf] (acex) at (4, -3) {A2C, PPO, SAC};
...
\end{tikzpicture}
-->
![RL 方法分类]({{ site.baseurl }}/assets/figures/rl-methods-tree.svg)
